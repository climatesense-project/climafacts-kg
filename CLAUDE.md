...
- **collectors/** fetch raw data (`skepticalscience.py` scrapes SkS myth/rebuttal/glossary pages; `cimplekg.py`/`climatesensekg.py` query external SPARQL endpoints) *and* run CARDS classification inline via `collectors/utils.py:batch_classify_cards_category` before writing back to the DB. `climatesensekg.py` reuses `cimplekg.py`'s `process_all`/`process_claims` directly — this only works because both SPARQL queries alias columns identically (`rev`/`date_published`/`text`); there is no declared collector interface, so a new source with different column names will silently break or write wrong data.
